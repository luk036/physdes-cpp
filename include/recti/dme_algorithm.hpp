...
public:
     ...
     */
    virtual std::pair<int, double> calculate_tapping_point(
        TreeNode& node_left, TreeNode& node_right, int distance) = 0;
};
...
public:
    ...
    double calculate_wire_delay_per_unit(double ) override {
    ...
    std::pair<int, double> calculate_tapping_point(
        TreeNode& node_left, TreeNode& node_right, int distance) override {
...
public:
    ...
    std::shared_ptr<TreeNode> build_clock_tree() {
        ...
        std::map<std::string, ManhattanArc<Interval<int>, Interval<int>>> merging_segments = compute_merging_segments(merging_tree);
...
private:
    ...
    std::shared_ptr<TreeNode> build_merging_tree(
        ...
        std::vector<std::shared_ptr<TreeNode>> left_group(sorted_nodes.begin(), sorted_nodes.begin() + mid);
        std::vector<std::shared_ptr<TreeNode>> right_group(sorted_nodes.begin() + mid, sorted_nodes.end());
        ...
        auto parent = std::make_shared<TreeNode>(
            "n" + std::to_string(node_id++), left_child->position);
    ...
    std::map<std::string, ManhattanArc<Interval<int>, Interval<int>>> compute_merging_segments(
        ...
        std::function<ManhattanArc<Interval<int>, Interval<int>>(const std::shared_ptr<TreeNode>&)> compute_segment;
        compute_segment = [&](const std::shared_ptr<TreeNode>& node) -> ManhattanArc<Interval<int>, Interval<int>> {
            if (node->is_leaf()) {
    ...
    std::shared_ptr<TreeNode> embed_tree(
        ...
        std::function<void(const std::shared_ptr<TreeNode>&, const ManhattanArc<Interval<int>, Interval<int>>*)> embed_node;
        embed_node = [&](const std::shared_ptr<TreeNode>& node, const ManhattanArc<Interval<int>, Interval<int>>* parent_segment) {
            if (!node) return;
...
inline TreeStatistics get_tree_statistics(const std::shared_ptr<TreeNode>& root) {
    ...
    std::function<void(const std::shared_ptr<TreeNode>&, const std::shared_ptr<TreeNode>&)> traverse;
...
inline void example_dme_usage() {
    ...
    // Create clock sinks
    std::vector<Sink> sinks = {
        Sink("s1", Point<int>(10, 20), 1.0),
        Sink("s2", Point<int>(30, 40), 1.0),
        Sink("s3", Point<int>(50, 10), 1.0),
        Sink("s4", Point<int>(70, 30), 1.0),
