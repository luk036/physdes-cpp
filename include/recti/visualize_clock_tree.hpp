...
namespace recti {
    ...
    class ClockTreeVisualizer {
      ...
      public:
        ...
        std::string visualize_tree(const std::shared_ptr<TreeNode>& root,
            ...
            svg_content.push_back("<svg width=\"" + std::to_string(width) + "\" height=\""
     ...
     * @param[in] height The height of the SVG canvas
     */
    inline std::string create_interactive_svg(const std::shared_ptr<TreeNode>& root,
                                              const std::vector<Sink>& sinks,
     ...
     * @param[in] filename The output filename for the SVG file
     */
    inline std::string create_delay_model_comparison(const TreeComparisonData& linear_tree_data,
