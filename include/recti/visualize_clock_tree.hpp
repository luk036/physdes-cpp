...
namespace recti {
     ...
     */
    inline auto join_strings(const std::vector<std::string>& strings, const std::string& delimiter) -> std::string {
        std::string result;
    ...
    public:
        ...
        auto visualize_tree(
            ...
            int width = 800,
            int height = 600,
            const std::optional<std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>>& analysis = std::nullopt
        ) -> std::string {
            ...
            float scale_x = (max_x > min_x) ? (width - 2 * margin_) / (max_x - min_x) : 1.0f;
            float scale_y = (max_y > min_y) ? (height - 2 * margin_) / (max_y - min_y) : 1.0f;
            float scale = std::min(scale_x, scale_y);

            auto scale_coord = [this, min_x, min_y, scale](float x, float y) -> std::pair<float, float> {
                return {(x - min_x) * scale + margin_, (y - min_y) * scale + margin_};
            };
    ...
    private:
         ...
         */
        auto calculate_bounds(const std::vector<const TreeNode*>& nodes, const std::vector<Sink>& sinks)
            -> std::tuple<float, float, float, float> {
            std::vector<std::pair<float, float>> all_points;
            for (const auto* node : nodes) {
                all_points.emplace_back(static_cast<float>(node->position().xcoord()), static_cast<float>(node->position().ycoord()));
            }
            for (const auto& sink : sinks) {
                all_points.emplace_back(static_cast<float>(sink.position().xcoord()), static_cast<float>(sink.position().ycoord()));
            }
            ...
            float min_x = std::min_element(all_points.begin(), all_points.end(),
                [](const auto& a, const auto& b) { return a.first < b.first; })->first;
            float max_x = std::max_element(all_points.begin(), all_points.end(),
                [](const auto& a, const auto& b) { return a.first < b.first; })->first;
            float min_y = std::min_element(all_points.begin(), all_points.end(),
                [](const auto& a, const auto& b) { return a.second < b.second; })->second;
            float max_y = std::max_element(all_points.begin(), all_points.end(),
                [](const auto& a, const auto& b) { return a.second < b.second; })->second;
         ...
         */
        auto draw_wires(const TreeNode* root, const std::function<std::pair<float, float>(float, float)>& scale_coord)
            -> std::vector<std::string> {
            std::vector<std::string> svg_elements;
            std::function<void(const TreeNode*)> draw_wires_recursive = [&](const TreeNode* node) {
                if (!node) return;
                if (node->parent()) {
                    auto [x1, y1] = scale_coord(static_cast<float>(node->parent().value()->position().xcoord()),
                    ...
                    svg_elements.push_back(
                        ...
                        "\" stroke=\"" + wire_color_ + "\" stroke-width=\"" + std::to_string(wire_width_) +
                    ...
                    if (node->wire_length() > 0) {
                        svg_elements.push_back(
                            "<text x=\"" + std::to_string(mid_x) + "\" y=\"" + std::to_string(mid_y - 5) +
        ...
        auto draw_nodes(const TreeNode* root, const std::vector<Sink>& sinks,
            ...
            std::function<void(const TreeNode*, int)> draw_nodes_recursive = [&](const TreeNode* node, int depth) {
                if (!node) return;

                auto [x, y] = scale_coord(static_cast<float>(node->position().xcoord()), static_cast<float>(node->position().ycoord()));
                bool is_sink = sink_positions.count({node->position().xcoord(), node->position().ycoord()}) > 0;
                ...
                svg_elements.push_back(
                    "<circle cx=\"" + std::to_string(x) + "\" cy=\"" + std::to_string(y) +
                    "\" r=\"" + std::to_string(radius) + "\" fill=\"" + color +
                    "\" stroke=\"#333\" stroke-width=\"1\"/>");

                float label_y_offset = static_cast<float>(-radius - 5);
                svg_elements.push_back(
                    "<text x=\"" + std::to_string(x) + "\" y=\"" + std::to_string(y + label_y_offset) +
                    "\" class=\"node-label\" text-anchor=\"middle\">" + node->name() + "</text>");

                float delay_y_offset = static_cast<float>(radius + 12);
                svg_elements.push_back(
                    "<text x=\"" + std::to_string(x) + "\" y=\"" + std::to_string(y + delay_y_offset) +
                    "\" class=\"delay-label\" text-anchor=\"middle\">d:" + std::to_string(node->delay()) + "</text>");

                if (is_sink) {
                    float cap_y_offset = static_cast<float>(radius + 22);
                    svg_elements.push_back(
                        "<text x=\"" + std::to_string(x) + "\" y=\"" + std::to_string(y + cap_y_offset) +
                        "\" class=\"delay-label\" text-anchor=\"middle\">c:" + std::to_string(node->capacitance()) + "</text>");
                }
         ...
         */
        auto create_analysis_box(const std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>& analysis)
            -> std::vector<std::string> {
            std::string delay_model = std::holds_alternative<std::string>(analysis.at("delay_model"))
                ? std::get<std::string>(analysis.at("delay_model")) : "Unknown";
            ...
            std::vector<std::string> analysis_text = {
                ...
                "Total Wirelength: " + std::to_string(std::get<float>(analysis.at("total_wirelength"))),
                "Sinks: " + std::to_string(std::get<std::vector<float>>(analysis.at("sink_delays")).size())
            };
            ...
            for (size_t i = 1; i < analysis_text.size(); ++i) {
                analysis_box.push_back("<tspan x=\"20\" y=\"" + std::to_string(45 + i * 16) + "\">" + analysis_text[i] + "</tspan>");
            }
    ...
    inline auto create_interactive_svg(
        const TreeNode* root,
        const std::vector<Sink>& sinks,
        const std::optional<std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>>& analysis,
        const std::string& filename = "clock_tree_interactive.svg",
        int width = 1000,
        int height = 700
    ) -> std::string {
        ClockTreeVisualizer visualizer(
            60, 10, 3, "#2E7D32", "#1565C0", "#C62828", "#455A64", "#263238");
    ...
    inline auto create_comparison_visualization(
        const std::vector<std::unordered_map<std::string, std::variant<const TreeNode*, std::vector<Sink>, std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>, std::string>>>& trees_data,
        ...
        for (size_t i = 0; i < num_trees; ++i) {
            ...
            svg_content.push_back(
                "<text x=\"" + std::to_string(offset_x + sub_width / 2) + "\" y=\"" + std::to_string(offset_y + 20) +
            ...
            auto temp_svg = visualizer.visualize_tree(
                ...
                std::get<std::vector<Sink>>(trees_data[i].at("sinks")),
                "",
                sub_width - 20,
                sub_height - 40,
                std::get<std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>>(trees_data[i].at("analysis")));
            ...
            for (size_t j = 0; j < lines.size(); ++j) {
                ...
                } else if (start_idx != -1 && lines[j].find("</g>") != std::string::npos && lines[j-1].find("clock-tree") == std::string::npos) {
            ...
            std::vector<std::string> tree_content(lines.begin() + start_idx + 1, lines.begin() + end_idx);
            if (!tree_content.empty() && tree_content[0].find("<rect width=\"100%\" height=\"100%\" fill=\"white\"/>") != std::string::npos) {
                tree_content.erase(tree_content.begin());
            }

            svg_content.push_back("<g transform=\"translate(" + std::to_string(offset_x + 10) + ", " + std::to_string(offset_y + 40) + ")\">");
    ...
    inline auto create_delay_model_comparison(
        const std::unordered_map<std::string, std::variant<const TreeNode*, std::vector<Sink>, std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>, std::string>>& linear_tree_data,
        const std::unordered_map<std::string, std::variant<const TreeNode*, std::vector<Sink>, std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>, std::string>>& elmore_tree_data,
