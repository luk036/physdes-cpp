...
- Replace all occurrences of "Recti" in the relevant CMakeLists.txt with the name of your project
  - Capitalization matters here: `Recti` means the name of the project, while `recti` is used in file names.
  - Remember to rename the `include/recti` directory to use your project's lowercase name and update all relevant `#include`s accordingly.
...
To collect code coverage information, run CMake with the `-DRECTI_ENABLE_COVERAGE=1` option.
...
```bash
cmake -B build -DRECTI_BUILD_DOCS=ON
...
# run standalone
./build/Recti --help
# build docs (requires -DRECTI_BUILD_DOCS=ON at configure time)
cmake --build build --target GenerateDocs
```
...
clang-tidy can be enabled by configuring CMake with `-DRECTI_ENABLE_CLANG_TIDY=ON` and building the `clang-tidy` target.
It analyzes the public headers using the checks configured in `.clang-tidy`.

```bash
cmake -B build -DRECTI_ENABLE_CLANG_TIDY=ON
