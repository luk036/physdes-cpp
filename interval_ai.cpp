...
template <typename T> class Interval {
  public:
    ...
    bool operator<(const T &other) const { return _ub < other; }
    bool operator>(const T &other) const { return _lb > other; }
    bool overlaps(const Interval<T> &other) const { return !(_ub < other._lb || other._ub < _lb); }
    bool contains(const T &obj) const { return _lb <= obj && obj <= _ub; }
    bool contains(const Interval<T> &obj) const { return _lb <= obj._lb && obj._ub <= _ub; }
...
template <typename T> bool overlap(const T &lhs, const T &rhs) {
...
template <typename T> bool contain(const T &lhs, const T &rhs) {
