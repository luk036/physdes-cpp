...
TEST_CASE("Point") {
    ...
    SUBCASE("Construction and Accessors") {
        CHECK_EQ(point1.xcoord(), 1);
        CHECK_EQ(point1.ycoord(), 2);
    }
    ...
    SUBCASE("Arithmetic Operators") {
        ...
        CHECK_EQ(point4.xcoord(), 2);
        CHECK_EQ(point4.ycoord(), 3);
        CHECK_EQ(point4, point3);

        point4 -= vector1;
        CHECK_EQ(point4, point1);

        const Point point5 = point3 - vector1;
        CHECK_EQ(point5, point1);
    }

    SUBCASE("Flip") {
        const Point point_flipped = point1.flip_xy();
        CHECK_EQ(point_flipped.xcoord(), 2);
        CHECK_EQ(point_flipped.ycoord(), 1);

        const Point point_flipped_y = point1.flip_y();
        CHECK_EQ(point_flipped_y.xcoord(), -1);
        CHECK_EQ(point_flipped_y.ycoord(), 2);
    }
    ...
    SUBCASE("Overlaps, Contains, Intersects, Hull") {
        ...
        CHECK_EQ(intersection.ycoord(), Interval(1, 2));

        const Point hull = point_interval1.hull_with(point_interval2);
        CHECK_EQ(hull.xcoord(), Interval(0, 3));
        CHECK_EQ(hull.ycoord(), Interval(0, 3));
    }
...
TEST_CASE("Point nearest_to") {
    ...
    CHECK_EQ(point_a.nearest_to(point_b), Point(3, 4));

    const Point rectangle(Interval(3, 4), Interval(5, 6));  // Rectangle
    CHECK_EQ(rectangle.nearest_to(point_a), Point(3, 5));
    CHECK_EQ(rectangle.nearest_to(point_b), Point(4, 6));
}
...
TEST_CASE("Point class") {
    ...
    SUBCASE("Constructor and accessors") {
        CHECK_EQ(point_a.xcoord(), 3);
        CHECK_EQ(point_a.ycoord(), 4);
    }
    SUBCASE("measure") {
        CHECK_EQ(point_a.measure(), 1);
        Point<int, int> point_b(3, 8);
        CHECK_EQ(point_b.measure(), 1);
    }
    SUBCASE("rotates") {
        auto rotated = point_a.rotates();
        CHECK_EQ(rotated.xcoord(), -1);
        CHECK_EQ(rotated.ycoord(), 7);
    }
    SUBCASE("inv_rotates") {
        Point<int, int> point_b(-1, 9);
        auto inv_rotated = point_b.inv_rotates();
        CHECK_EQ(inv_rotated.xcoord(), 4);
        CHECK_EQ(inv_rotated.ycoord(), 5);
    }
    SUBCASE("blocks") {
        Point<int, int> point_b(5, 6);
        CHECK_EQ(point_a.blocks(point_b), false);
        // Add more complex cases with Intervals if needed
    }
    SUBCASE("nearest_to") {
        Point<int, int> point_b(5, 6);
        auto nearest = point_a.nearest_to(point_b);
        CHECK_EQ(nearest.xcoord(), 3);
        CHECK_EQ(nearest.ycoord(), 4);
        Point<Interval<int>, Interval<int>> rectangle(Interval<int>(3, 4), Interval<int>(5, 6));
        auto nearest1 = rectangle.nearest_to(point_a);
        CHECK_EQ(nearest1.xcoord(), 3);
        CHECK_EQ(nearest1.ycoord(), 5);
        auto nearest2 = rectangle.nearest_to(point_b);
        CHECK_EQ(nearest2.xcoord(), 4);
        CHECK_EQ(nearest2.ycoord(), 6);
    }
    SUBCASE("get_center") {
        CHECK_EQ(point_a.get_center().xcoord(), 3);
        CHECK_EQ(point_a.get_center().ycoord(), 4);
        Point<Interval<int>, int> point_b(Interval<int>(3, 7), 4);
        CHECK_EQ(point_b.get_center().xcoord(), 5);
        CHECK_EQ(point_b.get_center().ycoord(), 4);
    }
    SUBCASE("lower_corner") {
        CHECK_EQ(point_a.lower_corner().xcoord(), 3);
        CHECK_EQ(point_a.lower_corner().ycoord(), 4);
        Point<Interval<int>, int> point_b(Interval<int>(3, 7), 4);
        CHECK_EQ(point_b.lower_corner().xcoord(), 3);
        CHECK_EQ(point_b.lower_corner().ycoord(), 4);
    }
    SUBCASE("upper_corner") {
        CHECK_EQ(point_a.upper_corner().xcoord(), 3);
        CHECK_EQ(point_a.upper_corner().ycoord(), 4);
        Point<Interval<int>, int> point_b(Interval<int>(3, 7), 4);
        CHECK_EQ(point_b.upper_corner().xcoord(), 7);
        CHECK_EQ(point_b.upper_corner().ycoord(), 4);
    }
}