...
TEST_SUITE("Clock Tree Visualization Tests") {
    TEST_CASE("ClockTreeVisualizer class") {
        ...
        SUBCASE("visualize_tree") {
            ...
            std::vector<Sink> sinks
                = {Sink("s1", Point<int, int>(10, 20)), Sink("s2", Point<int, int>(30, 40))};
    ...
    //         = {Sink("s1", Point<int, int>(10, 20)), Sink("s2", Point<int, int>(30, 40))};
    //     std::unordered_map<std::string, std::variant<float, std::string, std::vector<float>>>
    //         analysis = {{"skew", 0.1f}, {"max_delay", 5.0f}, {"total_wirelength", 100.0f}};
